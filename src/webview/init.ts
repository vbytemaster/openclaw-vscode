...
import { normalizeChats } from './state/chatStore.js';
...
export function restoreInitialState(ctx: InitContext): void {
  ...
  if (state && Array.isArray(state.chats)) {
    ctx.setChats(normalizeChats(normalizeChatIds(state.chats)));
    ...
    ctx.setChats(normalizeChats(normalizeChatIds(fileData.chats)));
    const chats = ctx.getChats();
    ...
    ctx.setChats([{
      ...
      messages: [],
      msgIndex: 0,
      agentId: ctx.agentSel?.value || 'main',
    }]);
    ctx.setActiveChatId('chat-1');
  }
